...
function getProjectCategory(projectId: string): "work" | "home" | "study" | "health" {
	switch (projectId) {
		case "1": return "work";
		case "2": return "home";
		case "3": return "study";
	...
	return (
		<div className="p-8 space-y-6 max-w-6xl">
			...
			<div>
				...
				<h2 className="text-[26px] font-semibold text-foreground tracking-tight leading-tight">Bom dia, Marcos 👋</h2>
				<p className="text-sm text-muted-foreground mt-1">
					Você tem <strong className="text-foreground font-semibold">{pendingToday} tarefas</strong> para concluir hoje
			...
			<div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
				{stats.map(({ key, label, value, icon: Icon, iconBg, iconColor, trend, trendVariant, sublabel, stripeClass, tintedBg, showArc }) => (
					<div
						key={key}
						className={cn(
							"relative rounded-[14px] p-5 overflow-hidden bg-card border border-border shadow-sm",
							tintedBg && "bg-[#FFFAF4] dark:bg-amber-950/20 border-amber-200/50 dark:border-amber-800/30"
						)}
					>
						{/* Left accent stripe */}
						<div className={cn("absolute left-0 top-0 bottom-0 w-[3px] rounded-l-[14px]", stripeClass)} />
						<div className="flex items-center justify-between mb-3">
							<div className={cn("flex items-center justify-center w-[30px] h-[30px] rounded-md", iconBg)}>
								{key === "ok" ? (
							...
							<span
								className={cn(
									...
									trendVariant === "up" && "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-400",
									trendVariant === "down" && "bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-400",
			...
			<div className="grid grid-cols-1 lg:grid-cols-[1fr_400px] gap-6 items-start">
				...
				<div className="bg-card border border-border rounded-[14px] shadow-sm overflow-hidden">
					<div className="flex items-center justify-between px-5 py-4 border-b border-border/70">
						<h3 className="text-[13px] font-semibold text-foreground">Tarefas de Hoje</h3>
						<button
							type="button"
				...
				<div className="bg-card border border-border rounded-[14px] shadow-sm overflow-hidden">
					<div className="flex items-center justify-between px-5 py-4 border-b border-border/70">
						<h3 className="text-[13px] font-semibold text-foreground">Projetos</h3>
						<button
							type="button"
								...
								>
									<div className="flex items-start gap-3 mb-3">
										...
										<span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-400 shrink-0">
											{p.status === "ativo" ? "Ativo" : p.status === "concluido" ? "Concluído" : "Arquivado"}
										</span>
									</div>
									<div className="flex items-center gap-3">
										<ProgressBar value={doneTasks} max={Math.max(projectTasks.length, 1)} color={progressColor} />
										<span
											className={cn(
