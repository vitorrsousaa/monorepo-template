...
import type { GetTodayTasksResponse, TodayProjectDto } from "@repo/contracts/tasks/today";
...
export type TodayTasksResponseWithOptimisticState = Omit<
	...
	"projects"
> & {
	projects: Array<Omit<TodayProjectDto, "tasks"> & { tasks: TaskWithOptimisticState[] }>;
};
			...
			queryFn: async () => {
				...
				const projects = todayTasks.projects.map(project => ({
					...
					tasks: project.tasks.map(task => ({
						...task,
						optimisticState: (task as TaskWithOptimisticState)?.optimisticState ?? OptimisticState.SYNCED,
					})),
				}));
